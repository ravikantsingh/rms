...
$msg='';
if($field=='sname')
$sql="SELECT * FROM `think_original` WHERE `sname` LIKE ? ";
else if($field=='mobile')
$sql="SELECT * FROM `think_original` WHERE `mobile` LIKE ? ";
else if($field=='email')
$sql="SELECT * FROM `think_original` WHERE `email` LIKE ? ";
else if($field=='fname')
$sql="SELECT * FROM `think_original` WHERE `fname` LIKE ? ";
else if($field=='enrollment')
$sql="SELECT * FROM `think_original` WHERE `enrollment` LIKE ? ";
...
foreach( $res as $row )
			...
			if($row['status']==0)
					 ...
					 $status='Payment Done';
		  $sqlCheck=$dbh->prepare("SELECT * FROM `batch_type` WHERE `batch_id`= ? ");
		 $runCheck=$sqlCheck->execute(array($row['batch']));
		 $row_batch=$sqlCheck->fetch();
			 ...
			 $roww=$sqlCheck->fetch();
			{
				...
				$msg.='
                                                <tr class="tabledown">
                                                    ...
                                                    <td id=""><a href="javascript:void(0)" style="cursor: text !important" onclick="studentInfo()">'.$course_name.'</a></td>
                                                    <td id=""><a href="javascript:void(0)" style="cursor: text !important" onclick="studentInfo()">'.$batch_name.'</a></td>
