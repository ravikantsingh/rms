...
function zgetExpenseData( $dbh, $sdate, $edate){
    $data = '';
    $sql = $dbh->prepare("SELECT * FROM `daily_expense` WHERE  `deleted` != 1 AND `date`>= ? AND `date`<= ? AND `user_id`= ?");
	$run = $sql->execute(array($sdate,$edate,$_SESSION['user_id']));
...
/*************************************************************************/
    ...
    $mode_total=0;
    $expense_total=0;
    $sql=$dbh->prepare("SELECT * FROM `order_details` WHERE `date`>= ? AND `date`<= ? AND `user_id`= ?");

	$run=$sql->execute( array( $from,$to,$_SESSION['user_id'] ) );
